...
class MacroeconomicAnalysisAgent:
    def __init__(self):
        ...
        self.system_prompt = SystemMessage(content=(
            ...
            "1. 환율, 금리, 원자재 가격 등 각 지표간의 상호 연관성을 분석하세요.\n"
    ...
    def _parse_market_data(self, soups):
        """파싱된 데이터에서 필요한 정보 추출"""
        data = {}
        ...
        for section, items in sections.items():
            ...
            for item in items:
                if item in market_data:
                    ...
                    formatted_text += f"{item}: {data['price']} (변동: {data['change']})\n"
...
if __name__ == "__main__":
    ...
    # 예시 쿼리
    test_query = "현재 거시경제 지표들이 한국 주식 시장에 미치는 영향을 분석하고, 이를 바탕으로 한 투자 전략을 제시해주세요."
