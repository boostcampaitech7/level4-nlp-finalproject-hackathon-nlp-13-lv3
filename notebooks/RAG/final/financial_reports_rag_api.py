    ...
    def check_groundedness(self, answer: str, query: str) -> Tuple[bool, int, str]:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                        "content": "당신은 평가 기준에 따라 매우 엄격하게 Groundedness Check을 하는 전문가입니다."},
            ...
            logger.info(
                f"Structured Groundedness evaluation response: {evaluation_text}")
...
# Initialize RAG system
        ...
        try:
            ...
            prompt = PromptTemplate(
                ...
                input_variables=["original_query",
                                 "previous_answer", "groundedness_explanation"]
            )
            ...
            logger.info(
                f"Rewritten query with previous answer context: {new_query}")
            return new_query
    ...
    def combine_answers(self, previous_answer: str, new_answer: str, original_query: str) -> str:
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content="당신은 금융 분석 보고서 작성 전문가입니다. 당신의 역할을 여러 답변을 통합하여 질문에 대해 정확한 답변하는 것입니다."),
                HumanMessage(content=formatted_prompt)
            ]
        ...
        while iteration < max_iterations:
            try:
                ...
                if accumulated_answer:
                    accumulated_answer = self.combine_answers(
                        accumulated_answer, current_answer, query)
                else:
                    accumulated_answer = current_answer

                # Groundedness Check - 원래 질문과 누적된 답변을 비교
                passed, score, explanation = self.check_groundedness(
                    accumulated_answer, query)
                logger.info(
                    f"Iteration {iteration + 1}: Groundedness score = {score}")
                ...
                if passed:
                    ...
                    iteration += 1
                    logger.info(
                        f"Iteration {iteration}: {score}점으로 답변의 근거성이 부족하여 질문 재작성 시도.")
                    previous_explanation = explanation
                    current_query = self.rewrite_query(
                        query, accumulated_answer, previous_explanation)
